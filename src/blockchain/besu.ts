/**
 * Hyperledger Besu blockchain integration for DID-VC platform
 *
 * Supports:
 * - DID registration on Besu (via DIDRegistry smart contract)
...
export class BesuService {
  ...
  private explorerUrl: string | null;
  ...
  constructor() {
    this.network = process.env.BESU_NETWORK || 'dev';
    this.rpcUrl = process.env.BESU_RPC_URL || 'http://localhost:8545';
    this.explorerUrl = process.env.BESU_EXPLORER_URL || null;
    this.didRegistryAddress = process.env.DID_REGISTRY_ADDRESS || null;
    this.vcRegistryAddress = process.env.VC_REGISTRY_ADDRESS || null;
    this.demoMode = !process.env.BESU_PRIVATE_KEY || !this.didRegistryAddress;

    if (!this.demoMode) {
      this.initializeContracts();
    } else {
      console.log('[Besu] Running in DEMO mode - transactions will be simulated');
    }
  }
  ...
  private initializeContracts() {
    try {
      ...
      this.signer = new ethers.Wallet(process.env.BESU_PRIVATE_KEY!, this.provider);
      ...
      console.log(`[Besu] Connected to ${this.network} @ ${this.rpcUrl} | DID: ${this.didRegistryAddress} | VC: ${this.vcRegistryAddress}`);
    } catch (error) {
      console.error('[Besu] Failed to initialize contracts, falling back to demo mode:', error);
      this.demoMode = true;
    }
  }
  ...
  getTxExplorerUrl(txHash: string): string | null {
  ...
  async registerDID(did: string, publicKeyHex: string): Promise<{ txHash: string; blockNumber?: number }> {
    if (this.demoMode) {
      ...
      console.log(`[Besu DEMO] DID registered: ${did} | Mock TX: ${mockTx}`);
    ...
    try {
      ...
      console.log(`[Besu] DID registered on-chain: ${did} | TX: ${tx.hash}`);
      return { txHash: tx.hash, blockNumber: receipt.blockNumber };
    } catch (error: any) {
      if (error.message?.includes('DID already registered')) {
        return { txHash: 'already_registered' };
      }
      throw new Error(`Failed to register DID on Besu: ${error.message}`);
    }
  }

  async resolveDIDFromChain(did: string): Promise<OnChainDIDInfo | null> {
    if (this.demoMode) return demoDidStore.get(did) || null;
    ...
    try {
      ...
      return { didString, publicKeyHex, controller, active, createdAt: Number(createdAt), updatedAt: Number(updatedAt) };
    } catch { return null; }
  }

  async isDIDActive(did: string): Promise<boolean> {
    if (this.demoMode) return demoDidStore.get(did)?.active || false;
  ...
  async anchorVC(
    ...
    if (this.demoMode) {
      ...
      console.log(`[Besu DEMO] VC anchored: ${vcId} | Hash: ${vcHash.slice(0, 20)}... | Mock TX: ${mockTx}`);
    ...
    try {
      ...
      const tx = await this.vcRegistry.issueVC(vcId, vcHashBytes, issuerDid, holderDid, credentialType, expiresAtTimestamp);
      const receipt = await tx.wait();
      console.log(`[Besu] VC anchored on-chain: ${vcId} | TX: ${tx.hash}`);
      return { txHash: tx.hash, vcHash, blockNumber: receipt.blockNumber };
    } catch (error: any) {
      throw new Error(`Failed to anchor VC on Besu: ${error.message}`);
    }
  }
  ...
  async verifyVCOnChain(vcId: string, vcJson: object): Promise<VCVerificationResult> {
    ...
    try {
      ...
      console.error('[Besu] verifyVC error:', error);
      return { hashValid: false, isRevoked: false, isExpired: false, exists: false, onChain: false };
    }
  }
  ...
  async isVCRevoked(vcId: string): Promise<boolean> {
    if (this.demoMode) return demoVCStore.get(vcId)?.revoked || false;
  ...
  createDIDFromAddress(address: string): string {
    return `did:besu:${this.network}:${address.toLowerCase()}`;
  }
}

// Singleton export
let _instance: BesuService | null = null;
export function getBesuService(): BesuService {
  if (!_instance) _instance = new BesuService();
  return _instance;
}
